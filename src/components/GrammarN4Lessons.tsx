...
export default function GrammarN4Lessons({ onGoBack }: GrammarN4LessonsProps) {
  ...
  useEffect(() => {
    localStorage.setItem("sk_n4_mastered_ids", JSON.stringify([...masteredIds]));
  }, [masteredIds]);
        ...
        <DetailView 
          ...
          onToggleLearned={() => {
            ...
            setMasteredIds(prev => {
              ...
              if (next.has(selectedGrammarId)) {
                next.delete(selectedGrammarId);
              } else {
                next.add(selectedGrammarId);
              }
              return next;
            });
          }}
    ...
    setTimeout(() => {
      setCardsQueue(prev => {
        if (remembered) {
          ...
          // Push to the back of the queue to study again
    ...
    return (
      <div className="space-y-6 animate-fadeIn">
        ...
        <div className="text-center pt-6">
          <button 
            onClick={onResetAll}
            className="text-xs font-black text-gray-400 hover:text-red-600 transition-colors uppercase tracking-widest underline"
          >
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex justify-between items-center bg-[#FDFBF7] p-4 rounded-xl border border-gray-200">
        <span className="font-bold text-[#1A1A1A] text-sm">Còn lại: <span className="text-[#8B0000]">{cardsQueue.length}</span> thẻ</span>
        <button 
          onClick={() => {
            playSound.click();
      ...
      {/* Feedback controller buttons */}
      <div className="grid grid-cols-2 gap-4 pt-4">
        <button
          disabled={!isFlipped}
          onClick={() => handleCardFeedback(false)}
          className={`py-4 rounded-xl font-bold transition-all border-2 border-[#1A1A1A] shadow-[3px_3px_0px_#1A1A1A] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none ${isFlipped ? "bg-rose-500 text-white hover:bg-rose-600" : "bg-gray-100 text-gray-300 border-gray-200 cursor-not-allowed shadow-none"}`}
        >
          CHƯA THUỘC ✕
        </button>
        <button
          ...
          className={`py-4 rounded-xl font-bold transition-all border-2 border-[#1A1A1A] shadow-[3px_3px_0px_#1A1A1A] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none ${isFlipped ? "bg-emerald-500 text-white hover:bg-emerald-600" : "bg-gray-100 text-gray-300 border-gray-200 cursor-not-allowed shadow-none"}`}
        >
          ĐÃ THUỘC ✓
        </button>
      </div>
