...
export default function Dashboard({ progress, vocabList, updateProgress, onNavigate }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    let g5 = 0;
    try {
      const saved = localStorage.getItem("sonkuro_n5_grammar_progress_v1");
      if (saved) {
        const parsed = JSON.parse(saved);
        g5 = Array.isArray(parsed.rememberedFlashcards) ? parsed.rememberedFlashcards.length : 0;
      }
    } catch (e) {}
    ...
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved);
        k5 = Array.isArray(parsed.learnedIds) ? parsed.learnedIds.length : 0;
      }
    } catch (e) {}
    ...
    let k4 = 0;
    try {
      const saved = localStorage.getItem("n4_known_kanji");
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed)) {
          k4 = parsed.length;
    ...
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved);
        k3 = Array.isArray(parsed.viewedKanjis) ? parsed.viewedKanjis.length : 0;
      }
    } catch (e) {}
    ...
    let v5 = 0;
    try {
      const saved = localStorage.getItem("n5_srs_v8");
      if (saved) {
        const parsed = JSON.parse(saved);
        for (const id in parsed) {
          const item = parsed[id];
          const repCount = Array.isArray(item) ? item[3] : item?.repCount;
          if (repCount > 0) {
            v5++;
          }
        }
    ...
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved);
        v4 = Object.values(parsed).filter(status => status === "mastered").length;
      }
    } catch (e) {}
    ...
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved);
        v3 = Object.values(parsed).filter(status => status === "mastered").length;
      }
    } catch (e) {}
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Limit to 800KB to be absolutely safe for Firestore doc limit (1MB max document size)
    if (file.size > 800 * 1024) {
      alert("Kích thước ảnh quá lớn! Em hãy chọn ảnh nhỏ hơn 800KB để lưu trữ đám mây ổn định nhé.");
      return;
    }
