...
export default function VocabN4Lessons({ onGoBack }: VocabN4LessonsProps) {
  ...
  const updateWordStatus = (wordId: number, newStatus: "new" | "learning" | "mastered") => {
    const updated = { ...wordStates, [wordId]: newStatus };
    setWordStates(updated);
    
    // Save only non-new items to preserve storage size
    const progressToSave: Record<number, "new" | "learning" | "mastered"> = {};
    Object.keys(updated).forEach((key) => {
      const id = Number(key);
      if (updated[id] !== "new") {
        progressToSave[id] = updated[id];
      }
    });
            ...
            <div className="space-y-6">
              ...
              >
                ...
                >
                  {/* Front Face */}
                  <div 
                    className="absolute inset-0 bg-white border-4 border-[#1A1A1A] rounded-3xl p-8 flex flex-col justify-center items-center text-center shadow-[6px_6px_0px_#1A1A1A]"
                    style={{ backfaceVisibility: "hidden" }}
                  >
                    <button 
                      onClick={(e) => { e.stopPropagation(); speakJapanese(studyQueue[studyIndex].kanji); }}
                      className="absolute top-4 right-4 p-2 bg-gray-100 hover:bg-gray-200 rounded-full transition-all border-2 border-[#1A1A1A]"
                      title="Nghe phát âm"
                    >
                    ...
                    <div className="space-y-4">
                      <div className="text-5xl font-black text-[#1A1A1A] leading-normal" style={{ fontFamily: "'Noto Serif JP', serif" }}>
                        {studyQueue[studyIndex].kanji}
                      </div>
                      <div className="text-xl font-bold text-gray-500">
                        {studyQueue[studyIndex].kana}
                      </div>
                      <div className="text-xs text-gray-400 font-bold animate-pulse pt-4">
                  ...
                  {/* Back Face */}
                  <div 
                    className="absolute inset-0 bg-[#FDFBF7] border-4 border-[#1A1A1A] rounded-3xl p-8 flex flex-col justify-center items-center text-center shadow-[6px_6px_0px_#1A1A1A]"
                    style={{ 
                  ...
                  >
                    ...
                    <div className="space-y-4 w-full">
                      <div className="text-sm font-black text-gray-400 border-b border-gray-200 pb-2">
                        Ý nghĩa & Ngữ cảnh
                      </div>
                      <div className="text-2xl font-black text-[#8B0000]">
                        {studyQueue[studyIndex].meaning}
                      </div>
                      <div className="text-base text-gray-500 font-bold italic">
                        {studyQueue[studyIndex].kana}
                      </div>
                      ...
                      {studyQueue[studyIndex].collocation && (
                        <div className="pt-2">
                          ...
                          <div className="text-sm font-bold text-gray-800 bg-white border border-[#1A1A1A] p-2 rounded-xl inline-block mt-1">
                            {studyQueue[studyIndex].collocation}
                          </div>
                        </div>
                      )}
              ...
              <div className={`grid grid-cols-2 gap-4 pt-4 transition-opacity duration-300 ${isFlipped ? "opacity-100" : "opacity-40 pointer-events-none"}`}>
                <button
                  onClick={() => handleGradeCard("learning")}
                  className="bg-white border-2 border-[#1A1A1A] hover:bg-red-50 text-[#C0392B] font-black py-4 rounded-2xl shadow-[3px_3px_0px_#1A1A1A] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all flex flex-col items-center justify-center"
                >
                ...
                <button
                  ...
                  className="bg-[#2ECC71] border-2 border-[#1A1A1A] hover:bg-[#27AE60] text-white font-black py-4 rounded-2xl shadow-[3px_3px_0px_#1A1A1A] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all flex flex-col items-center justify-center"
                >
