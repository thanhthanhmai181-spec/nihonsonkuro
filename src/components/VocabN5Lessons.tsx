...
export default function VocabN5Lessons({ onGoBack }: VocabN5LessonsProps) {
  ...
  const [listFilterStatus, setListFilterStatus] = useState<"all" | "learned" | "unlearned">("all");
  ...
  useEffect(() => {
    ...
    // Also parse learnedWordIds from hoc_cung_thay_son_progress
    try {
      const savedProg = localStorage.getItem("hoc_cung_thay_son_progress");
      if (savedProg) {
        const parsedProg = JSON.parse(savedProg);
        if (Array.isArray(parsedProg.learnedWordIds)) {
          parsedProg.learnedWordIds.forEach((idStr: string) => {
    ...
    const handleStorageChange = (e: any) => {
      ...
      if (!key || key === 'n5_srs_v8' || key === 'hoc_cung_thay_son_progress') {
  ...
  // Save state safely merged with current localStorage & sync with hoc_cung_thay_son_progress
  const saveSRSState = (updatedSRS: Record<number, SRSItem>) => {
    let latestSaved: Record<string, any> = {};
    const savedSRS = localStorage.getItem('n5_srs_v8');
    if (savedSRS) {
      try { latestSaved = JSON.parse(savedSRS); } catch (e) {}
    }

    const dataToSave: Record<number, [number, string, number, number]> = { ...latestSaved };
    for (const id in updatedSRS) {
      const s = updatedSRS[id];
      const numId = Number(id);
      dataToSave[numId] = [s.interval, s.nextReview, s.efactor, s.repCount];
    }

    localStorage.setItem('n5_srs_v8', JSON.stringify(dataToSave));
    setSrsState(updatedSRS);

    // Sync to hoc_cung_thay_son_progress learnedWordIds
    try {
      const rawProg = localStorage.getItem('hoc_cung_thay_son_progress');
      if (rawProg) {
        const prog = JSON.parse(rawProg);
        let learnedArr: string[] = Array.isArray(prog.learnedWordIds) ? prog.learnedWordIds : [];
        let changed = false;

        for (const id in updatedSRS) {
          const s = updatedSRS[id];
          const numId = Number(id);
          const tag1 = `n5_${numId}`;
          const tag2 = `${numId}`;

          if (s.repCount > 0) {
            if (!learnedArr.includes(tag1)) { learnedArr.push(tag1); changed = true; }
            if (!learnedArr.includes(tag2)) { learnedArr.push(tag2); changed = true; }
          } else {
            if (learnedArr.includes(tag1) || learnedArr.includes(tag2)) {
              learnedArr = learnedArr.filter(x => x !== tag1 && x !== tag2);
              changed = true;
            }
          }
        }
        ...
        if (changed) {
          prog.learnedWordIds = learnedArr;
          localStorage.setItem('hoc_cung_thay_son_progress', JSON.stringify(prog));
        }
      }
    } catch (e) {}

    window.dispatchEvent(new CustomEvent("local-storage-changed", { detail: { key: 'n5_srs_v8' } }));
  };

  // Toggle single word learned/memorized state directly
  const handleToggleWordLearned = (wordId: number) => {
    playSound.click();
  ...
  const filteredWordsList = useMemo(() => {
    return vocabData.filter(v => {
      ...
      const s = srsState[v.id];
      const isLearned = s && s.repCount > 0;
      const matchesStatus = listFilterStatus === "all" || 
        (listFilterStatus === "learned" && isLearned) || 
        (listFilterStatus === "unlearned" && !isLearned);
      ...
      return matchesLesson && matchesStatus && matchesSearch;
    });
  }, [vocabData, listFilterLesson, listFilterStatus, listSearchQuery, srsState]);
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="bg-white p-4 border-2 border-[#1A1A1A] rounded-2xl shadow-[2px_2px_0px_#1A1A1A] flex flex-col sm:flex-row gap-4 items-center">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <span className="text-xs font-black text-slate-500 whitespace-nowrap">Trạng thái:</span>
              <select
                value={listFilterStatus}
                onChange={(e) => setListFilterStatus(e.target.value as any)}
                className="w-full px-3 py-2 bg-slate-50 border-2 border-slate-200 focus:border-[#8B0000] rounded-xl font-bold text-sm outline-none cursor-pointer"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredWordsList.slice(0, 100).map((v) => {
              ...
              return (
                <div 
                  key={v.id} 
                  className={`p-4 border-2 rounded-2xl transition flex justify-between items-center group relative overflow-hidden ${
                    isLearned
                      ? "bg-green-50/40 border-green-700 shadow-[3px_3px_0px_#15803d]"
                      : "bg-white border-[#1A1A1A] shadow-[3px_3px_0px_#1A1A1A] hover:border-[#8B0000]"
                  }`}
                >
                  <div className="space-y-1.5 flex-1 pr-3">
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                      ...
                      {isLearned && (
                        <>
                          <span>•</span>
                          <span className={isDue ? "text-[#8B0000]" : "text-green-600 font-black"}>
                            {isDue ? "🔴 Cần Ôn" : "🟢 Đã Thuộc"}
                          </span>
                        </>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      onClick={() => handleToggleWordLearned(v.id)}
                      className={`px-3 py-2 border-2 rounded-xl text-xs font-black transition cursor-pointer flex items-center gap-1.5 shadow-sm active:translate-x-0.5 active:translate-y-0.5 ${
                        isLearned
                          ? "bg-green-600 text-white border-green-800 hover:bg-green-700"
                          : "bg-white text-slate-700 border-slate-300 hover:border-[#8B0000] hover:text-[#8B0000]"
                      }`}
                      title={isLearned ? "Bỏ đánh dấu đã thuộc" : "Đánh dấu đã thuộc (Tích nhớ)"}
                    >
                      <Check className="w-3.5 h-3.5" />
                      <span>{isLearned ? "Đã thuộc" : "Tích nhớ"}</span>
                    </button>

                    <button
                      onClick={() => speak(v.kanji || v.hiragana)}
                      className="p-2.5 bg-slate-50 border border-slate-200 hover:bg-[#8B0000] hover:text-white rounded-xl transition cursor-pointer"
                      title="Phát âm"
                    >
