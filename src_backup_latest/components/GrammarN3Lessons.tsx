...
export default function GrammarN3Lessons({ onGoBack }: GrammarN3LessonsProps) {
  ...
  // Load progress on mount and storage updates
  useEffect(() => {
    ...
    const handleStorageChange = (e: any) => {
      ...
      if (!key || key === "n3_grammar_progress") {
        loadFromStorage();
      }
    };
  ...
  const saveProgress = (updated: typeof userProgress) => {
    let latestSaved: any = {};
    const savedStr = localStorage.getItem("n3_grammar_progress");
    if (savedStr) {
      try { latestSaved = JSON.parse(savedStr); } catch (e) {}
    }

    const mergedFlashcards = { ...(latestSaved.flashcards || {}), ...updated.flashcards };
    const mergedQuizzes = { ...(latestSaved.quizzes || {}), ...updated.quizzes };

    // Pick highest quiz score for each quiz
    Object.keys(updated.quizzes || {}).forEach(k => {
      const qId = Number(k);
      const oldScore = (latestSaved.quizzes || {})[qId] || 0;
      const newScore = updated.quizzes[qId] || 0;
      mergedQuizzes[qId] = Math.max(oldScore, newScore);
    });
    ...
    localStorage.setItem("n3_grammar_progress", JSON.stringify(merged));
  };
  ...
  // Keyboard navigation for GrammarN3 flashcards
  useEffect(() => {
  ...
  }, [activeTab, flashcardIndex, isFlipped]);
