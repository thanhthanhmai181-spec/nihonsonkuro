...
export default function ListeningShadowing({ onGoBack }: ListeningShadowingProps) {
  ...
  const [episodesList, setEpisodesList] = useState<ListeningEpisode[]>(() => {
    try {
      const saved = localStorage.getItem("custom_listening_episodes_v4");
      if (saved) {
        const parsed = JSON.parse(saved);
        if (Array.isArray(parsed) && parsed.length > 0) {
          // Merge preset episodes with latest data, and retain custom user-created episodes
          const presetMap = new Map(PRESET_LISTENING_EPISODES.map(ep => [ep.id, ep]));
          const mergedPresets = PRESET_LISTENING_EPISODES.map(preset => {
            return preset;
          });
          const customEpisodes = parsed.filter((ep: ListeningEpisode) => !presetMap.has(ep.id));
          return [...mergedPresets, ...customEpisodes];
        }
      }
    } catch (e) {}
  ...
  const [activeEpisode, setActiveEpisode] = useState<ListeningEpisode | null>(() => episodesList[0] || null);

  // Auto save episode list changes
  useEffect(() => {
    try {
      localStorage.setItem("custom_listening_episodes_v4", JSON.stringify(episodesList));
    } catch (e) {}
  }, [episodesList]);
  ...
  const activeYoutubeId = activeEpisode ? (customYoutubeMap[activeEpisode.id] || activeEpisode.youtubeId) : null;

  // Initialize YouTube listener and bidirectional state sync
  useEffect(() => {
      ...
      try {
        ...
        if (data.event === "infoDelivery" && data.info) {
          if (typeof data.info.currentTime === "number") {
            ...
            // If playing only a single targeted line, stop YouTube when reaching end time
            ...
            if (activeEpisode && activeEpisode.lines && activeEpisode.lines.length > 0) {
              const matchedIndex = activeEpisode.lines.findIndex((line, idx) => {
                const start = line.startTime ?? (idx * 4);
                const nextLine = activeEpisode.lines[idx + 1];
                const end = line.endTime ?? (nextLine?.startTime ?? start + 4);
                return time >= start && time < end;
              });

              if (matchedIndex !== -1 && matchedIndex !== currentLineIndex) {
                setCurrentLineIndex(matchedIndex);
                if (autoScrollSubtitles) {
    ...
    const initYT = () => {
      if ((window as any).YT && (window as any).YT.Player) {
        try {
          ...
          // Iframe API binding optional fallback
        }
      } else {
    ...
    const timer = setTimeout(initYT, 200);
  ...
  }, [activeYoutubeId, activeEpisode?.id, currentLineIndex, autoScrollSubtitles, postYoutubeCommand]);

  // Sync Subtitles Karaoke with YouTube Playback Time in Realtime
  useEffect(() => {
    if (!activeYoutubeId || !isPlaying) return;
  ...
  }, [activeYoutubeId, isPlaying, activeEpisode, currentLineIndex, autoScrollSubtitles, postYoutubeCommand]);
            ...
            <div className="bg-white border border-natural-border rounded-3xl p-5 sm:p-6 shadow-sm space-y-6">
              ...
              <div className="space-y-6">
                ...
                {activeYoutubeId && (
                  <div className="bg-slate-900 border border-slate-700 rounded-3xl p-4 sm:p-5 space-y-4 text-white shadow-xl">
                    ...
                    <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-black border border-slate-800 shadow-inner">
                      <iframe
                        ...
                        src={`https://www.youtube.com/embed/${activeYoutubeId}?enablejsapi=1&rel=0&modestbranding=1&playsinline=1&start=${activeEpisode?.lines?.[0]?.startTime || 0}`}
                  ...
                  {/* Karaoke Script Line-by-Line List with Mic & Scoring */}
