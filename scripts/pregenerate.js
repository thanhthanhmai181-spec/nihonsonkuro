...
async function run() {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (fs.existsSync(diskPath)) {
      console.log(`[${i + 1}/${items.length}] Cached: "${cleanText.substring(0, 15)}..." (${selectedVoice})`);
      continue;
    }

    console.log(`[${i + 1}/${items.length}] Generating AI Studio audio: "${cleanText.substring(0, 15)}..." (${selectedVoice})...`);
    ...
    if (rawData) {
      ...
      console.log(`  Saved ${hashFilename}`);
    } else {
