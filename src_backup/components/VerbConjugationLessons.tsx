...
export default function VerbConjugationLessons({ onGoBack }: VerbConjugationLessonsProps) {
  ...
  useEffect(() => {
    const handleStorageChange = (e: any) => {
      ...
      if (!key || key === "sonkuro_verb_stats") {
        const saved = localStorage.getItem("sonkuro_verb_stats");
        if (saved) {
          try {
            const parsed = JSON.parse(saved);
            setStats(prev => ({
              total: Math.max(prev.total || 0, parsed.total || 0),
              correct: Math.max(prev.correct || 0, parsed.correct || 0),
              streak: Math.max(prev.streak || 0, parsed.streak || 0),
              bestStreak: Math.max(prev.bestStreak || 0, parsed.bestStreak || 0),
              byCat: {
                verbs: {
                  total: Math.max(prev.byCat?.verbs?.total || 0, parsed.byCat?.verbs?.total || 0),
                  correct: Math.max(prev.byCat?.verbs?.correct || 0, parsed.byCat?.verbs?.correct || 0)
                },
                iadj: {
                  total: Math.max(prev.byCat?.iadj?.total || 0, parsed.byCat?.iadj?.total || 0),
                  correct: Math.max(prev.byCat?.iadj?.correct || 0, parsed.byCat?.iadj?.correct || 0)
                },
                naadj: {
                  total: Math.max(prev.byCat?.naadj?.total || 0, parsed.byCat?.naadj?.total || 0),
                  correct: Math.max(prev.byCat?.naadj?.correct || 0, parsed.byCat?.naadj?.correct || 0)
                },
                nouns: {
                  total: Math.max(prev.byCat?.nouns?.total || 0, parsed.byCat?.nouns?.total || 0),
                  correct: Math.max(prev.byCat?.nouns?.correct || 0, parsed.byCat?.nouns?.correct || 0)
                }
              }
            }));
          } catch (e) {}
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem("sonkuro_verb_stats");
    if (saved) {
      try { latestSaved = JSON.parse(saved); } catch (e) {}
    }

    const merged = {
      total: Math.max(stats.total || 0, latestSaved.total || 0),
      correct: Math.max(stats.correct || 0, latestSaved.correct || 0),
      streak: Math.max(stats.streak || 0, latestSaved.streak || 0),
      bestStreak: Math.max(stats.bestStreak || 0, latestSaved.bestStreak || 0),
      byCat: {
        verbs: {
          total: Math.max(stats.byCat?.verbs?.total || 0, latestSaved.byCat?.verbs?.total || 0),
          correct: Math.max(stats.byCat?.verbs?.correct || 0, latestSaved.byCat?.verbs?.correct || 0)
        },
        iadj: {
          total: Math.max(stats.byCat?.iadj?.total || 0, latestSaved.byCat?.iadj?.total || 0),
          correct: Math.max(stats.byCat?.iadj?.correct || 0, latestSaved.byCat?.iadj?.correct || 0)
        },
        naadj: {
          total: Math.max(stats.byCat?.naadj?.total || 0, latestSaved.byCat?.naadj?.total || 0),
          correct: Math.max(stats.byCat?.naadj?.correct || 0, latestSaved.byCat?.naadj?.correct || 0)
        },
        nouns: {
          total: Math.max(stats.byCat?.nouns?.total || 0, latestSaved.byCat?.nouns?.total || 0),
          correct: Math.max(stats.byCat?.nouns?.correct || 0, latestSaved.byCat?.nouns?.correct || 0)
        }
      }
    };

    localStorage.setItem("sonkuro_verb_stats", JSON.stringify(merged));
  }, [stats]);
