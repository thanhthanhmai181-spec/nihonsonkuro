...
export default function KanjiN4Lessons({ onGoBack }: KanjiN4LessonsProps) {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: any) => {
      ...
      if (!key || ["n4_known_kanji", "n4_known_examples", "n4_quiz_correct", "n4_quiz_total"].includes(key)) {
        loadFromStorage();
      }
    };
  ...
  // --- Helpers to save state cleanly merged ---
  const saveKnownKanjiSet = (newSet: Set<string>) => {
    let existingArr: string[] = [];
    const savedKanji = localStorage.getItem("n4_known_kanji");
    if (savedKanji) {
      try { existingArr = JSON.parse(savedKanji); } catch (e) {}
    }
    const combined = Array.from(new Set([...existingArr, ...Array.from(newSet)]));
    setKnownKanji(new Set(combined));
    localStorage.setItem("n4_known_kanji", JSON.stringify(combined));
  };
  ...
  const saveKnownExamplesSet = (newSet: Set<string>) => {
    ...
    const savedExamples = localStorage.getItem("n4_known_examples");
    if (savedExamples) {
      try { existingArr = JSON.parse(savedExamples); } catch (e) {}
    }
    const combined = Array.from(new Set([...existingArr, ...Array.from(newSet)]));
    setKnownExamples(new Set(combined));
    localStorage.setItem("n4_known_examples", JSON.stringify(combined));
  };

  const saveQuizStats = (correct: number, total: number) => {
    const oldCorrect = Number(localStorage.getItem("n4_quiz_correct") || 0);
    const oldTotal = Number(localStorage.getItem("n4_quiz_total") || 0);

    const mergedCorrect = Math.max(oldCorrect, correct);
    const mergedTotal = Math.max(oldTotal, total);

    setQuizCorrect(mergedCorrect);
    setQuizTotal(mergedTotal);
    localStorage.setItem("n4_quiz_correct", mergedCorrect.toString());
    localStorage.setItem("n4_quiz_total", mergedTotal.toString());
  };
