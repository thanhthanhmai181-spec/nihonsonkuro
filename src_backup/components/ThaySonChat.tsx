...
import { Send, Sparkles, MessageCircle, Volume2, RotateCcw, AlertTriangle, User, Key } from "lucide-react";
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      const sendRequest = async () => {
        return await fetch("/api/gemini/chat", {
          method: "POST",
      ...
      // If server returned rate limit error, auto retry once after 2 seconds
      if (!response.ok && data.error && (data.error.includes("quá nhiều") || data.error.includes("429") || data.error.includes("hạn mức"))) {
        console.warn("Rate limit detected on client, auto-retrying in 2 seconds...");
      ...
      if (!response.ok || data.error) {
        throw new Error(data.error || "Không thể kết nối đến server. Có thể chìa khóa Gemini API chưa được định cấu hình chính xác.");
      }
        ...
        <div className="bg-[#2D241E] text-white px-5 py-4 flex items-center justify-between border-b border-[#2D241E]">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`text-xs flex items-center gap-1 border px-2.5 py-1.5 rounded-xl cursor-pointer transition-colors ${
                getCustomGeminiKey() 
                  ? "bg-emerald-500/20 text-emerald-200 border-emerald-400/40 hover:bg-emerald-500/30" 
                  : "bg-white/5 text-natural-sand/80 hover:text-white border-white/10"
              }`}
              title="Cấu hình Gemini API Key cá nhân"
            >
              <Key className="w-3.5 h-3.5 text-pink-300" />
              <span className="hidden sm:inline">
                {getCustomGeminiKey() ? "API Key Cá Nhân" : "API Key riêng"}
              </span>
            </button>
          ...
          {error && (
            <div className="bg-rose-50 border border-rose-200 text-rose-800 p-4 rounded-2xl flex items-start gap-3 max-w-md mx-auto">
              ...
              <div className="space-y-1.5 flex-1">
                <span className="font-bold text-xs block">Lỗi kết nối AI</span>
                <p className="text-xs leading-relaxed">{error}</p>
                <button
                  type="button"
                  onClick={() => setIsKeyModalOpen(true)}
                  className="mt-1 text-[11px] font-bold text-pink-700 bg-pink-100 hover:bg-pink-200 px-3 py-1.5 rounded-lg inline-flex items-center gap-1.5 transition-colors cursor-pointer"
                >
                  <Key className="w-3.5 h-3.5" />
                  <span>Nhập API Key Gemini riêng để dùng không giới hạn</span>
                </button>
              </div>
            </div>
          )}
