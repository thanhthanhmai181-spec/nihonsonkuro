...
export interface UserLearnedStats {
  ...
  kanji: number;     // Tổng Hán tự đã thuộc (N5 + N4 + N3, tối đa 587)
  total: number;     // Tổng điểm kiến thức = Vocab + Grammar + Kanji (tối đa 3316)
...
export function calculateDetailedUserStats(
  ...
  const kanjiTotal = Math.min(587, k5 + k4 + k3);
